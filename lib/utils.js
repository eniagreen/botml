...
class Utils {
  ...
  interpolateVariables (text, { allowUndefined = true } = {}) {
    ...
    return text.replace(/\$([a-z][\w_-]*)(\.[.\w[\]]*[\w\]])/g, (match, variable, output) => {
      try {
        const result = this.context.variables.get(variable.toLowerCase()) // eslint-disable-line no-unused-vars
  ...
  evalCode (parser, code, returns = true) {
    ...
    const keys = Array.from(this.context.variables.keys())
    const vars = Object.assign({}, ...keys.map(key => {
      const keyStr = key.startsWith('$') ? key : `$${key}`
      return { [keyStr]: this.context.variables.get(key) }
    }))
    const args = ['context', ...Object.keys(vars)].join(', ')
    const varsValues = Object.values(vars)

    // eslint-disable-next-line no-useless-call
    return ((str, context) => {
      const evaled = returns ? `((${args}) => (\n${str}\n))` : `((${args}) => {\n${str}\n})`
      try {
        // eslint-disable-next-line no-eval, no-useless-call
        return eval(evaled).call(null, context, ...varsValues)
      } catch (e) {
