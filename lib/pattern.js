...
module.exports = {
  ...
  execPattern: (...args) => {
    // TODO Handle the inversion of arguments when:
    //   pattern.match ? pattern.match(input) : input.match(pattern)
    let captures = XRegExp.exec(...args)
    let keys = Object.keys(captures).filter(key => !['index', 'input'].includes(key))
