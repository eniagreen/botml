...
function hasDefaultAction (result) {
  this.context.dialogues.forEach(d => {
    if (d.label === '*') {
      result.push({ name: d.label, type: 'default', result: d.block.match(/^(\w*.*)$(?!\n)/mg)[0].trim() })
    }
  })
  return result
}

function actionsMapping (actionType) {
  switch (actionType.type) {
    case 'switch':
      const sNextActionsNames = this.context.switchModel.switchCases.map(c => { if (c.match(/^[><`] (.*)$/m) !== null) return c.match(/^[><`] (.*)$/m)[1] })
      const sNextActionResult = this.context.switchModel.switchCases.map(c => c.match(/^(\w*.*)$/mg).map(i => i.trim()))
      return hasDefaultAction.call(this, sNextActionsNames.map((a, i) => ({ name: a, type: actionType.type, result: sNextActionResult[i] })))
    case 'dialogue':
      const trigger = this.block.match(/^> (.*)$/m)[1]
      if (/^\[\w+\]$/.test(trigger)) {
        const list = this.context.lists.get(this.block.match(/^> \[(.*)\]$/m)[1]).block
        const interpolatedList = list.replace(/^- \[.*\]/mg, (match) =>
          this.context.lists.get(match.replace(/- \[(.*)\]/, '$1')).block.replace(/= .*\n/, '')
        )
        return hasDefaultAction.call(this, [{
          name: interpolatedList.match(/- (.*)/mg).map(n => n.replace(/- /mg, '')).join(' | '),
          type: actionType.type,
          result: this.block.match(/^(\w*.*)$/mg).map(i => i.trim())
        }])
      } else {
        return hasDefaultAction.call(this, [{
          name: this.block.match(/^> (.*)$/m)[1],
          type: actionType.type,
          result: this.block.match(/^(\w*.*)$/mg).map(i => i.trim())
        }])
      }
    case 'prompt':
      ...
      this.context.dialogues.forEach(d => {
        actionType.names.map(n => {
          if (d.label !== '*' && RegExp(`(?:^|[\\s,;—])(?:\\W*)(${d.label})(?:\\W*)(?!\\w)`, 'gi').test(n)) {
            actionsResults.push({ name: d.label, type: actionType.type, result: d.block.match(/^(\w*.*)$/mg).map(i => i.trim()) })
          }
        })
      })
      return hasDefaultAction.call(this, actionsResults)
  }
}
  ...
  states: {
    ...
    dialogue: {
      _onEnter: function () {
        ...
        this.emitter.emit('dialogue-actions', actionsMapping.call(this, { type: 'dialogue' }))
    ...
    prompt: {
      _onEnter: function () {
        ...
        this.emitter.emit('prompt-actions', actionsMapping.call(this, { type: 'prompt', names: processedReplies }))
    ...
    switch: {
      _onEnter: function () {
        ...
        this.emitter.emit('switch-actions', actionsMapping.call(this, { type: 'switch' }))
