...
class Utils {
  ...
  interpolateVariables (text) {
    return text.replace(/\$([a-z][\w_-]*)(\.[.\w[\]]*[\w\]])/g, (match, variable, output) => {
      ...
      return this.context.variables.get(variable.toLowerCase())
  ...
  evalCode (code, returns = false) {
    // eslint-disable-next-line no-useless-call
    return ((str) => {
      const evaled = returns ? `((this.context) => (${str}))` : `((this.context) => {${str}})`
      try {
        // eslint-disable-next-line no-eval, no-useless-call
        return eval(evaled).call(null, this.context)
      } catch (e) {
        log.warn('Error while running script', e, { evaled })
        return undefined
      }
    }).call(null, code, Object.assign({}, this.context, { say: this.stay, service: this.service, emit: emitter.emit }))
  }
}
