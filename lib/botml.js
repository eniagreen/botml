...
module.exports = class BotML {
  ...
  prompt () {
    // 0. Handle the case where one of the workflows must be activated and used
    //    by default when the user connects to the bot.
    if (context.workflows.size && !context.initialWorkflowStarted) {
      context.initialWorkflowStarted = true
    ...
    prompt(input => {
      ...
      let handle = (input, dialogue, label) => {
        ...
        dialogue.activators.filter(p => p.test(input)).some(pattern => {
          ...
          // a. unnamed captures
          let captures = pattern.match ? pattern.match(input) : input.match(pattern)
