  ...
  initialize: function (block, context, blockHistory = [{ type: 'init', remainingBlock: block }]) {
    ...
    this.switchModel = {
      ...
      captureCases: function (block) {
        ...
        this.switchCases = this.switchCases.map(sc => sc.replace(/~ \[.*\].*\n$/, `$&  ${this.parentWorkflowContinuing}`))
        this.switchStatement = true
      },
      captureParentWorkflowContinuing: function (block) {
        const parentWorkflowContinuing = block.match(/@ trigger\('nothing'\)(.*\n?)*/)
        if (parentWorkflowContinuing !== null) {
          console.log('block', block)
          console.log('parentWorkflowContinuing', parentWorkflowContinuing)
          this.parentWorkflowContinuing = removeBlockCurrentLine(parentWorkflowContinuing[0])
        }
      },
      ...
      reset: function () {
        ...
        this.parentWorkflowContinuing = null
        this.currentCase = 0
        this.switchCases = []
      }
    }
  ...
  states: {
    ...
    switch: {
      _onEnter: function () {
        ...
        this.context.switchModel.captureParentWorkflowContinuing(this.block)
