...
class Utils {
  ...
  service (name, output) {
    ...
    log.info('service', { name, url, output })
    ...
    console.log('service called:', name, JSON.stringify({ raw_result: result }).slice(0, 120))
    // eslint-disable-next-line no-eval
    result = output ? eval(`result${output}`) : result
    console.log('service called:', name, JSON.stringify({ result }).slice(0, 120))
