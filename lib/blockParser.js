...
let Parser = machina.Fsm.extend({
  ...
  states: {
      ...
      _onEnter: function () {
        ...
        try {
          message = message.replace(/`([^`]*)`/g, (m, script) =>
            this.utils.evalCode(this.utils.interpolateVariables(script))
          )
          if (message) {
    ...
    service: {
      _onEnter: function () {
        ...
        // Case 2. Service
          ...
          this.utils.service(label, output, (result) => {
    ...
    code: {
      _onEnter: function () {
        const result = this.utils.evalCode(this.utils.interpolateVariables(this.line), true)
    ...
    codeBlock: {
      _onEnter: function () {
        const result = this.utils.evalCode(this.utils.interpolateVariables(this.line))
        this.context.variables.set('$', result)
