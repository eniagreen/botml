...
let Parser = Fsm.extend({
  ...
  actionsMapping: function (actionType) {
    switch (actionType.type) {
      case 'switch':
        const switchNames = this.context.switchModel.switchCases.map(c => { if (c.match(/^[><`] (.*)$/m) !== null) return c.match(/^[><`] (.*)$/m)[1] })
      ...
      case 'dialogue':
        // if '> [list]'
        if (/^\[\w+\]$/.test(this.block.match(/^> (.*)$/m)[1])) {
      ...
      case 'prompt':
        ...
        this.context.workflows.forEach(w => {
          let name = w.block.match(/^> .*$/)
          if (name !== null) {
            name = name[0].replace(/^> /, '')
            actionType.names.map(n => {
              if (name !== '*' && RegExp(`(?:^|[\\s,;—])(?:\\W*)(${name})(?:\\W*)(?!\\w)`, 'gi').test(n)) {
                workflowActions.push({ name, type: actionType.type, result: w.block.match(/^(\w*.*)$/mg).map(i => i.trim()) })
              }
            })
          }
        })
      ...
      case 'all':
        ...
        this.context.workflows.forEach(w => {
          if (w.block.match(/^> /)) {
            allWorkflows.push({ name: w.block.match(/^> (.*)/)[1], type: 'workflow', result: w.block.match(/^(\w*.*)$/mg).map(i => i.trim()) })
          }
        })
