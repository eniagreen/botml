...
const { evalCode, interpolateLists, interpolateVariables, random, say, service } = require('./utils')
...
function interpolateReferencingWorkflow (text) {
  ...
  if (isReferencingWorkflow.test(text)) {
    ...
    if (workflow) {
      log.debug('_interpolateReferencingWorkflow', workflowRef)
      let lineToReplace = RegExp(`^\\s*~\\s*\\[${workflowRef}\\]\\s*$`, 'm')
      let workflowRawWithoutHeading = workflow.block.replace(/^\s*~[a-z](?:[a-z0-9-_]*[a-z0-9])?$\n/m, '')
      workflowRawWithoutHeading += '\n@ trigger(\'nothing\')' // hacky code that ensures the next action/line will be handled
                                                              // not as a response candidate but as a response
      return text.replace(lineToReplace, workflowRawWithoutHeading)
    } else {
      log.warn(`The workflow '${workflow}' does not exist.`)
    }
  }
  return text
}
 ...
 * States:
 * - uninitialized
 * - [ code | dialogue | output | prompt | service | workflow ]*
 * - terminated
 */
let Parser = machina.Fsm.extend({
  ...
  states: {
      ...
      _onEnter: function () {
        ...
        try {
          message = message.replace(/`([^`]*)`/g, (m, script) =>
            evalCode(interpolateVariables(script))
          )
          if (message) {
            say(message)
          }
        } catch (e) {
          log.warn('Error while running script', e)
        }
      ...
      _onEnter: function () {
        ...
        // Case 2. Service
          ...
          service(label, output, (result) => {
            ...
            log.warn(`Error while executing service '${label}'`, error)
            throw error
          })
        } else {
    ...
    code: {
      _onEnter: function () {
        const result = evalCode(interpolateVariables(this.line), true)
        context.variables.set('$', result)

        this.block = removeBlockCurrentLine(this.block)
        // hacky code that ensures the next action/line will be handled
        this.block = `@ trigger('nothing')\n${this.block}`
        this.next()
      }
    },
    ...
    codeBlock: {
      _onEnter: function () {
        const result = evalCode(interpolateVariables(this.line))
        context.variables.set('$', result)
    ...
    terminated: {
      _onEnter: () => log.debug('End of block')
    }
  },
