...
let mergeActionsNames = (block) =>
  block.match(/(>.*\n)*/m)[0].replace(/>/mg, '').replace(/\n /g, ' | ')

let getArrayOfLines = (block) =>
  block.match(/^(\w*.*)$/mg)

let filterActionsResults = (block, name) =>
  getArrayOfLines(block).map(i => i.trim()).splice(name.split(' | ').length - 1)

function interpolateActionsResults (results) {
  let wrapped
  let [...data] = results
  ...
  data = data.map(r => {
    const result = r[1]
    if (/^~ \[.*\]$/.test(result)) {
      const workflow = this.context.workflows.get(result.replace(/~ \[(.*)\]/, '$1'))
      if (workflow) {
        r[1] = getArrayOfLines(workflow.block)[0]
      } else {
        const checkpoint = loadCheckpoint(result, this.blockHistory)
        if (checkpoint) {
          r[1] = getArrayOfLines(checkpoint)[0]
        } else {
          const jumpto = getFromBrackets(result)
          const block = this.block.match(RegExp(`(^~ ${jumpto}\\n)(.*)`, 'mg'))
          if (block !== null) {
            r[1] = block[0].replace(`~ ${jumpto}\n`, '')
          } else {
            r[1] = getArrayOfLines(jumpTo(this.blockHistory, this.block))[0]
          }
        }
      }
    }
    if (/^~ (?!\[).*/.test(result)) r[1] = r[2]
    return r
  })
  return wrapped ? data[0] : data
}
...
const blockType = (block) => typeof block !== 'object' && TYPES[block.trimLeft()[0]]
...
let Parser = Fsm.extend({
  ...
  actionsMapping: function (actionType) {
    switch (actionType.type) {
      case 'switch':
        if (!this.context.switchModel.switchStatement) {
          this.block = removeBlockLinesOfType(this.block, '>')
          this.context.switchModel.reset()
          this.context.switchModel.captureCases(this.block)
          this.context.switchModel.captureSignType(this.block)
        }
        const cases = this.context.switchModel.switchCases
        const switchNames = cases.map((c, i) => {
          const list = c.match(/^> \[(.*)\]/)
          const dialogue = c.match(/^[>`] (.*)$/m)
          const defaultCase = c.match(/^[<`] (.*)$/m)
          if (list !== null) return this.context.lists.get(list[1]).value.join(' | ')
          if (dialogue !== null) return dialogue[1]
          if (defaultCase !== null && i === cases.length - 1) return '🛑default'
        })
        const switchResults = interpolateActionsResults.apply(this, [cases.map(c => getArrayOfLines(c).map(i => i.trim()))])
        return switchNames.map((a, i) => ({
          name: a,
          type: actionType.type,
          result: a === '🛑default'
            ? ['🛑default', ...switchResults[i]]
            : switchResults[i]
        }))
      case 'dialogue':
        ...
        if (/^\[\w+\]$/.test(block.match(/^> (.*)$/m)[1])) {
          ...
          return [{
            ...
            type: actionType.type,
            result: interpolateActionsResults.apply(this, [getArrayOfLines(block).map(i => i.trim())])
          }]
        // if '> word'
        } else {
          const name = mergeActionsNames(block)
          const result = interpolateActionsResults.apply(this, [filterActionsResults(block, name)])
          return [{
            name,
            type: actionType.type,
            result
      ...
      case 'prompt':
        ...
        this.context.dialogues.forEach(d => {
          actionType.names.map(n => {
            if (RegExp(`(?:^|[\\s,;—])(?:\\W*)(${d.label[0] === '*' ? '\\' : ''}${d.label})(?:\\W*)(?!\\w)`, 'gi').test(n)) {
              const name = mergeActionsNames(d.block)
              const result = interpolateActionsResults.apply(this, [filterActionsResults(d.block, name)])
              dialogueActions.push({ name, type: actionType.type, result })
            }
          })
        })
        // check workflows
          ...
          if (name !== null) {
            ...
            actionType.names.map(n => {
              if (RegExp(`(?:^|[\\s,;—])(?:\\W*)(${/^\*/.test(name) ? '\\' : ''}${name})(?:\\W*)(?!\\w)`, 'gi').test(n)) {
                const name = mergeActionsNames(w.block)
                const result = interpolateActionsResults.apply(this, [filterActionsResults(w.block, name)])
                workflowActions.push({ name, workflowName: w.label, type: actionType.type, result })
              }
            })
          }
        })
      ...
      case 'all':
        ...
        this.context.dialogues.forEach(d => {
          const name = mergeActionsNames(d.block)
          const result = interpolateActionsResults.apply(this, [filterActionsResults(d.block, name)])
          allDialogues.push({ name, type: 'dialogue', result })
        })
        // catch all workflows
        this.context.workflows.forEach(w => {
          if (w.block.match(/^> /)) {
            const name = mergeActionsNames(w.block)
            const result = interpolateActionsResults.apply(this, [filterActionsResults(w.block, name)])
            allWorkflows.push({ name, workflowName: w.label, type: 'workflow', result })
          }
        })
