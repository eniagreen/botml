...
let Parser = machina.Fsm.extend({
  ...
  states: {
    ...
    prompt: {
      _onEnter: function () {
        ...
        replies.value.map(l => {
          (/\[.+\]/.test(l))
            ? finalReplies.push(...this.context.lists.get(l.match(/^\[([^\]]+)\]$/)[1]).value)
            : finalReplies.push(l)
        })

        log.info('[parser]', chalk.dim('smart replies:'), finalReplies.map(s => `[${s}]`).join(chalk.dim(', ')))
        this.emitter.emit('smart-replies', finalReplies)
    ...
    code: {
      _onEnter: function () {
        ...
        if (this.context.switchModel.type === 'code' && this.context.switchModel.switchStatement) {
          if (!result) {
            this.block = this.context.switchModel.switchCases[this.context.switchModel.currentCase++]
            this.markBlockHistory({ type: 'switch', event: 'code:setDefaultCase', remainingBlock: this.block })
  ...
  activators: function (forceReloading = false) {
    ...
    if (!forceReloading && this._activators) {
