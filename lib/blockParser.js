...
let getSwitchCases = (block, cases) => 
  cases.map(c => block.replace(/^\s*---\s*\n(([^\n]*\n)+)\s*---\s*\n(.*\n*)*/, removeBlockCurrentLine(c)))
  ...
  states: {
    ...
    switch: {
      _onEnter: function () {
        ...
        let capturedCases = this.block.match(/(---\n( {2}.+\n)*)/g)
        let captureDefaultCase = this.block.match(/(---\n(\S.+\n?)+)/g)

        capturedCases.pop()
        capturedCases.push(captureDefaultCase[0])
        switchModel.switchCases = getSwitchCases(this.block, capturedCases)
        
        log.info('[parser]', chalk.dim('switch:', JSON.stringify(switchModel)))
        this.context = Object.assign(this.context, { switchModel })

        // trigger the next instruction
        this.block = this.block.replace(/^\s*---\s*\n(([^\n]*\n)+)\s*---\s*\n(.*\n*)*/, removeBlockCurrentLine(capturedCases[this.context.switchModel.currentCase++]))
        this.markBlockHistory({ type: 'switch', event: 'removeBlockCurrentLine', remainingBlock: this.block })
        this.next()
      }
    },
