...
class Utils {
  ...
  service (name, output) {
    ...
    url = encodeURI(url)
    log.info('[utils]', 'service', { name, url, output })
    ...
    log.info('[utils]', 'service called:', name, JSON.stringify({ raw_result: result }).slice(0, 120))
    // eslint-disable-next-line no-eval
    result = output ? eval(`result${output}`) : result
    log.info('[utils]', 'service called:', name, JSON.stringify({ result }).slice(0, 120))
