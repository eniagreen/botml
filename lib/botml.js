...
class BotML {
  ...
  send (input) {
    log.info('[botml]', chalk.bold(`> ${input}`))
    // reset
    ...
    let handle = (input, dialogue, label) => {
      ...
      dialogue.activators().filter(p => RegExp(p.source, p.flags).test(input)).some(pattern => {
        this.context.switchModel.switchStatement = false
        this.context.switchModel.startIteration = false
    ...
    if (!this.context.saidSomething && this.context.switchModel.switchStatement) {
      ...
      while (this.context.switchModel.switchStatement) {
