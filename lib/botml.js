const fs = require('fs'),
      ...
      { debug, info, inspect, prompt, random, stats, warn } = require('./utils'),
      { context, emitter } = require('./context'),
...
module.exports = class BotML {
  ...
  start() {
    emitter.emit('bot-start');
    this.prompt();
  }

  prompt() {
    if (currentDialogue) emitter.emit('bot-update', currentDialogue);
