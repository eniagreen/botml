  ...
  _updateText (text) {
    ...
    switch (this.type) {
      case 'dialogue':
        ...
        this.activators = activators && activators[0].split(/\s*>\s*/).filter(s => s).map(patternify)
      ...
      case 'service':
        ...
        if (!service) service = this.label.match(/^(\w+)\s*\(([^\)]+)\)(\.[\.\w\[\]]+)?\s*$/i)
    ...
    switch (this.rawType) {
      ...
      case '@':
        ...
        if (this.label === 'trigger') {
          this.raw.replace(/^\s*@\s*trigger\('([^']+)'(?:\s*,\s*(.*))?\)$/m, (match, eventName, value) => {
