...
let filterActionsResults = (block, name) =>
  block.match(/^(\w*.*)$/mg).map(i => i.trim()).splice(name.split(' | ').length - 1)
  ...
  actionsMapping: function (actionType) {
    switch (actionType.type) {
      case 'switch':
        ...
        const switchResults = cases.map(c => c.match(/^(\w*.*)$/mg).map(i => i.trim()))
      ...
      case 'dialogue':
        ...
        // if '> word'
          ...
          const result = filterActionsResults(block, name)
          return [{
            name,
            type: actionType.type,
            result
        ...
        this.context.dialogues.forEach(d => {
          actionType.names.map(n => {
            if (RegExp(`(?:^|[\\s,;—])(?:\\W*)(${d.label === '*' ? '\\' : ''}${d.label})(?:\\W*)(?!\\w)`, 'gi').test(n)) {
              ...
              const result = filterActionsResults(d.block, name)
          ...
          if (name !== null) {
            ...
            actionType.names.map(n => {
              if (RegExp(`(?:^|[\\s,;—])(?:\\W*)(${/^\*/.test(name) ? '\\' : ''}${name})(?:\\W*)(?!\\w)`, 'gi').test(n)) {
                ...
                const result = filterActionsResults(w.block, name)
