...
class BotML {
    ...
    if (input.startsWith('/')) {
      switch (input) {
        ...
        case '/stats':
          ...
          emitter.emit('debug', this.utils.stats())
          break
        case '/inspect':
        ...
        case '/current':
          if (this.currentDialogue) {
            ...
            emitter.emit('debug', { currentDialogue: this.currentDialogue })
          } else {
  ...
  _handleDialogue (dialogue) {
    ...
    if (!this.currentDialogue.dialogue.remaining()) {
      emitter.emit('current-dialogue-end', this.currentDialogue.label)
      this.currentDialogue = undefined
    }
