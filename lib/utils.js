...
function debug() {
  ...
  if (process.env.debug) console.log(_chalk2.default.dim('DEBUG: ' + something.join(' | ')));
}
...
function trackServiceVariables(serviceName, props) {
  (0, _keys2.default)(props).forEach(function (key) {
    var value = props[key];
    _context2.default.variables.set('last_service_' + key, value);
    _context2.default.variables.set('service_' + serviceName + '_' + key, value);
  });
}
...
function service(name, output, onsuccess, onerror) {
  ...
  debug('service', { name: name, url: url, output: output });
  trackServiceVariables(name, { name: name, url: url, output: output });
  try {
    var body = (0, _child_process.execSync)('curl -s --compressed "' + url + '"', { timeout: 4000 });
    var result = JSON.parse(body);
    trackServiceVariables(name, { raw_result: result });
    console.log('service called:', name, { result: result });
    result = output ? eval('result' + output) : result; // eslint-disable-line no-eval
    console.log('service called:', name, { result: result });
    trackServiceVariables(name, { result: result });
