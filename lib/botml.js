...
const { inspect, prompt, stats } = require('./utils')
    ...
    if (input.startsWith('/')) {
      switch (input) {
        ...
        case '/stats':
          ...
          break
        case '/inspect':
          log.info(chalk.bold('variables'), inspect('variables'))
          log.info(chalk.bold('workflows'), inspect('workflows'))
          break
        case '/block':
          if (currentDialogue) {
            log.info(currentDialogue.dialogue.block)
          }
          break
        case '/activators':
          log.debug('current dialogue activators:')
          if (currentDialogue) {
            log.info(currentDialogue.dialogue.activators().join(' , '))
          }
          log.debug('global dialogue activators:')
          context.dialogues.forEach(dialogue => {
            log.info(dialogue.activators().join(' , '))
          })
          break
        case '/current':
          if (currentDialogue) {
