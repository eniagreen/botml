...
const { interpolateLists, interpolateVariables, random, say, service } = require('./utils')
...
let Parser = machina.Fsm.extend({
  ...
  states: {
    ...
    code: {
      _onEnter: function () {
        try {
          const result = eval(interpolateVariables(this.line)) // eslint-disable-line no-eval
