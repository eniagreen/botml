...
class Botml {
  ...
  async load (files = []) {
    console.time('loaded in') // eslint-disable-line no-console
    ...
    files.forEach(async file => {
      if (file.startsWith('http://') || file.startsWith('https://')) {
        await this._loadURL(file)
      } else if (IS_NODE_ENV && fs.lstatSync(file).isDirectory()) {
    ...
    log.debug(chalk.dim(`${this.utils.stats()} `))
    console.timeEnd('loaded in') // eslint-disable-line no-console
  }
  ...
  start (withPrompt = false) {
    ...
    // Emit start-actions event
    this.emitter.emit('all-actions', this.context.actionsMapping.call(this, { type: 'all' }))
  ...
  send (input) {
    ...
    if (!this.context.saidSomething && this.context.switchModel && this.context.switchModel.switchStatement) {
    ...
    // nothing
    if (!this.context.saidSomething) {
      this.emitter.emit('no-dialogue', 'No matching dialogue found.')
...
const IS_NODE_ENV = typeof process !== 'undefined' && process && process.versions && process.versions.node
